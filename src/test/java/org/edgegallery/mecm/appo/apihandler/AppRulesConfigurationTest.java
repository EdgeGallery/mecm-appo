...
public class AppRulesConfigurationTest {
    ...
    private void instantiateAppInstanceFlowUrls(MockRestServiceServer server, String appInstanceId) {
        ...
        // Mocking get mepm API
        url = "http://2.2.2.2:10000/apprulemgr/v1/tenants/" + TENANT_ID + APP_INSTANCE + appInstanceId
            + "/appd_configuration";
        server.expect(requestTo(url)).andExpect(method(HttpMethod.POST)).andRespond(withSuccess(
            "{\"taskId\": \"\",\"appInstanceId\": \"40519ee1-fb9d-4a61-855c-1b5c2a41da9f\","
                + "\"detailed\": \"duplicate dns entry\",\"configResult\": \"FAILURE\"}", MediaType.APPLICATION_JSON));

        // Mocking get mepm API
        url = "http://10.9.9.1:11111/inventory/v1/tenants/" + TENANT_ID + APP_INSTANCE + appInstanceId
            + "/appd_configuration";
        server.expect(requestTo(url)).andExpect(method(HttpMethod.GET)).andRespond(withStatus(HttpStatus.NOT_FOUND));
    ...
    private void appRulesUpdateFlowUrls(MockRestServiceServer server, String appInstanceId) {
        ...
        server.expect(requestTo(url)).andExpect(method(HttpMethod.POST)).andRespond(withSuccess(
            "{\"taskId\": \"\",\"appInstanceId\": \"40519ee1-fb9d-4a61-855c-1b5c2a41da9f\","
                + "\"detailed\": \"duplicate dns entry\",\"configResult\": \"SUCCESS\"}", MediaType.APPLICATION_JSON));
    ...
    public void instantiateTerminateTest() throws Exception {
        ...
        ResultActions postResult = mvc.perform(MockMvcRequestBuilders.post(APPO_TENANT + TENANT_ID + "/app_instances")
            .contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON).with(csrf()).content(
                "{ \"appPackageId\": \"f20358433cf8eb4719a62a49ed118c9b\", \"appName\": " + "\"face_recognitionRule\", "
                    ...
                    + "\"appInstanceDescription\": \"face_recognitionRule\", " + "\"mecHost\": \"2.2.2.2\" }")
        ...
        // Configure app rules
        ResultActions postConfigAppRuleResult = mvc.perform(
            MockMvcRequestBuilders.post(APPO_TENANT + TENANT_ID + APP_INSTANCE + appInstanceId + "/appd_configuration")
