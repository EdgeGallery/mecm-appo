...
public class ResourceMgrNetworkTest {

    private static final Logger LOGGER = LoggerFactory.getLogger(ResourceMgrNetworkTest.class);
    ...
    private static final String NETWORK_ID = "1234";
    ...
    private void networksInfos(MockRestServiceServer server){

        // Mocking get mepm API
        String url =  RESOURCE_CTRL_URL  + "/networks";
        server.expect(requestTo(url))
                .andExpect(method(HttpMethod.GET))
                .andRespond(withSuccess("{\n\"data\": null,\n\"retCode\": 0,\n\"message\": \"query networks " +
                        "success\",\n\"params\": null\n}", MediaType.APPLICATION_JSON));
    }
    ...
    @Test
    @WithMockUser(roles = "MECM_TENANT")
    public void queryNetworks() throws Exception {

        inventoryInfos(server);
        networksInfos(server);

        // Create a queryFlavors
        ResultActions getResult =
                mvc.perform(MockMvcRequestBuilders.get(RESOURCE_MGR + "/networks")
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    @Test
    @WithMockUser(roles = "MECM_TENANT")
    public void queryNetworksById() throws Exception {

        inventoryInfos(server);

        // Mocking get mepm API
        String url1 =  RESOURCE_CTRL_URL  + "/networks/"
                + NETWORK_ID;
        server.expect(requestTo(url1))
                .andExpect(method(HttpMethod.GET))
                .andRespond(withSuccess("{\n\"data\": null,\n\"retCode\": 0,\n\"message\": \"queryById network " +
                        "success\",\n\"params\": null\n}", MediaType.APPLICATION_JSON));

        // Create a queryFlavors By Id
        ResultActions getResult =
                mvc.perform(MockMvcRequestBuilders.get(RESOURCE_MGR + "/networks/" + NETWORK_ID)
                        .contentType(MediaType.APPLICATION_JSON)
                        .accept(MediaType.APPLICATION_JSON)
                        .header(ACCESS_TOKEN, SAMPLE_TOKEN));
        MvcResult getMvcResult = getResult.andDo(MockMvcResultHandlers.print())
                ...
                .andReturn();
        String postResponse = getMvcResult.getResponse().getContentAsString();
        LOGGER.info("response networkId: {}",postResponse);
        assertNotNull(postResponse);
    }

    @Test
    @WithMockUser(roles = "MECM_TENANT")
    public void deleteNetworksById() throws Exception {
        inventoryInfos(server);
        ...
        ResultActions deleteResult =
                mvc.perform(MockMvcRequestBuilders.delete(RESOURCE_MGR + "/networks/" + NETWORK_ID)
                        .contentType(MediaType.APPLICATION_JSON)
                        .accept(MediaType.APPLICATION_JSON).with(csrf())
                        .header(ACCESS_TOKEN, SAMPLE_TOKEN));
        ...
        LOGGER.info("response deleted networkId: {}",deleteResponse);
